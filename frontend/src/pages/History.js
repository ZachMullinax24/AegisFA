...
const SEV_DOT = {
  critical: 'sev-critical', high: 'sev-high', medium: 'sev-medium', low: 'sev-low', none: 'sev-none',
};
...
export default function History() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      getDetections(orgId, selectedId),
    ]).then(([a, d]) => {
      setAnalysis(a);
      setDetections(d || []);
    }).catch(() => {
      ...
      <div style={{ flex: 1, padding: '1.75rem', maxWidth: '1200px', width: '100%', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.75rem', flexWrap: 'wrap', gap: '1rem' }} className="fade-in">
          <div>
            ...
            <p style={{ fontFamily: 'var(--mono)', fontSize: '11px', color: 'var(--muted)' }}>
              // analysis results for your organization's log files
            </p>
          </div>
          <select className="select" value={selectedId} onChange={e => setSelectedId(e.target.value)} style={{ minWidth: '280px' }}>
            <option value="">— select a log file —</option>
            {files.map(f => (
              <option key={f.id} value={f.id}>{f.filename} ({f.source_type}) — {f.status}</option>
            ))}
          </select>
        </div>

        {/* File list when none selected */}
        {!selectedId && (
        ...
        {selectedId && (
          loadingAnalysis ? (
              ...
              <div className="card">
                ...
                <div style={{ padding: '1.25rem' }}>
                  ...
                  {analysis.attack_vector && (
                    <p style={{ fontFamily: 'var(--mono)', fontSize: '11px', color: 'var(--muted)', marginTop: '8px' }}>
                      Attack vector: <span style={{ color: 'var(--amber)' }}>{analysis.attack_vector}</span>
                    </p>
                  )}
                </div>
              </div>
                    ...
                    <div key={i} style={{ padding: '12px 18px', borderBottom: '1px solid var(--surface2)', display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        {f.indicators?.length > 0 && (
                          <p style={{ fontFamily: 'var(--mono)', fontSize: '10px', color: 'var(--muted)', marginTop: '6px' }}>
            ...
            <div className="card fade-in">
              <div className="empty-state">No analysis found. The file may still be processing.</div>
            </div>
          )
        )}
      </div>
